...
void ItemPreFrame(struct local_state_s *from, struct local_state_s *to, struct usercmd_s *usercmd, int runfuncs, double time, unsigned int random_seed)
{
	if (runfuncs)
	{
		...
		{
			if (bAliveLocal()) delay = GetTickCount();
...
void DrawWeaponName()
{
	...
	if (cvar.visual_weapon_local && bAliveLocal() && ent && ent->curstate.weaponmodel)
	{
		model_s* mdl = g_Studio.GetModelByIndex(ent->curstate.weaponmodel);
		if (mdl && GetTickCount() - delay < 5000)
		{
			ImVec2 Screen;
			...
			ImGui::GetCurrentWindow()->DrawList->AddRect({ Screen.x - 2, Screen.y + 50 }, { Screen.x + label_size + 3, Screen.y + 64 }, Wheel1());
			ImGui::GetCurrentWindow()->DrawList->AddText({ Screen.x, Screen.y + 50 }, White(), weapon);
		}
	}
}
