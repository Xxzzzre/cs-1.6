...
void ImGui::RenderSquare(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float scale)
{
    ...
    a = ImVec2(+0.750f, +0.866f) * r;
    b = ImVec2(-0.750f, -0.866f) * r;
    draw_list->AddRectFilled(center + a, center + b, col);
}

// Render an arrow aimed to be aligned with text (p_min is a position in the same space text would be positioned). To e.g. denote expanded/collapsed state
void ImGui::RenderArrowDouble(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale)
{
    const float h = draw_list->_Data->FontSize * 1.00f;
    float r = h * 0.40f * scale;
    ImVec2 center = pos + ImVec2(h * 0.50f, h * 0.50f * scale);
    ...
    case ImGuiDir_Right:
        if (dir == ImGuiDir_Left) r = -r;
        a1 = ImVec2(+1.500f, +0.000f) * r;
        b1 = ImVec2(-0.000f, +0.866f) * r;
        c1 = ImVec2(-0.000f, -0.866f) * r;
        a2 = ImVec2(+0.000f, +0.000f) * r;
        b2 = ImVec2(-1.500f, +0.866f) * r;
        c2 = ImVec2(-1.500f, -0.866f) * r;
        break;
    case ImGuiDir_Up:
    case ImGuiDir_Down:
    case ImGuiDir_None:
    ...
    draw_list->AddTriangleFilled(center + a1, center + b1, center + c1, col);
    draw_list->AddTriangleFilled(center + a2, center + b2, center + c2, col);
}
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    RenderArrow(bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, size.y - g.FontSize - g.Style.FramePadding.y)), dir);

    return pressed;
}

bool ImGui::ArrowButtonExDouble(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderArrowDouble(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col, dir);

    return pressed;
}

bool ImGui::SquareButtonEx(const char* str_id, ImVec2 size, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, bg_col, true, g.Style.FrameRounding);
    RenderSquare(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col);

    return pressed;
}

bool ImGui::ArrowButton(const char* str_id, ImGuiDir dir, float width)
{
    float sz = GetFrameHeight();
    return ArrowButtonEx(str_id, dir, ImVec2(width, sz), 0);
}

bool ImGui::SquareButton(const char* str_id, float width)
{
    float sz = GetFrameHeight();
    return SquareButtonEx(str_id, ImVec2(width, sz), 0);
}

bool ImGui::ArrowButtonDouble(const char* str_id, ImGuiDir dir, float width)
{
    float sz = GetFrameHeight();
    return ArrowButtonExDouble(str_id, dir, ImVec2(width, sz), 0);
}
...
bool ImGui::DragInt7(const char* label, int v[7], float v_speed, int v_min, int v_max, const char* format)
{
    return DragScalarN(label, ImGuiDataType_S32, v, 7, v_speed, &v_min, &v_max, format);
}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = CalcItemSize(size_arg, CalcItemWidth() + 1, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y*2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
...
bool ImGui::InputInt7(const char* label, int v[7], ImGuiInputTextFlags extra_flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 7, NULL, NULL, "%d", extra_flags);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : CalcItemWidth() + 1;
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id, &frame_bb))
...
bool ImGui::Combo(const char* label, float* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
	ImGuiContext& g = *GImGui;
...
bool ImGui::Combo(const char* label, float* current_item, const char* const items[], int items_count, int height_in_items)
{
	const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);
	return value_changed;
}
...
bool ImGui::MenuItem(const char* label, const char* shortcut, float* p_selected, bool enabled)
{
...
void ImGui::Separator()
{
	...
	float x1 = window->Pos.x + 6;
	float x2 = window->Pos.x + window->Size.x - 6;
	...
	for (int i = 0; i < windowsize; i++)
	{
		...
		window->DrawList->AddLine(ImVec2(bb.Min.x + windowsize / windowsize * i , bb.Min.y), ImVec2(bb.Min.x + windowsize / windowsize * (i + 1), bb.Max.y), GetColorU32(ImGuiCol_Separator));
		ImGui::PopStyleColor();
	}
